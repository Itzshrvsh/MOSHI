...
def _tool_label(tool):
    raw = str(tool or "unknown tool")
    name = raw.split("/")[-1].split(".")[-1]
    mapping = {
        "bash": "💻 Running PowerShell",
        "read": "📖 Reading files",
        "write": "📝 Creating files",
        "edit": "🛠️ Editing files",
        "glob": "🔎 Searching files",
        "grep": "🔍 Searching code",
        "task": "🤖 Delegating agent task",
        "todowrite": "📋 Updating task plan",
        "todoread": "📋 Reading task plan",
    }
    return mapping.get(name.lower(), f"⚙️ Running {name}")


def _render_status(chat_id):
    ...
    phase = task_phase.get(chat_id, "Starting")
    tool = task_tool.get(chat_id)
    files = list(dict.fromkeys(task_files.get(chat_id, [])))
    command = task_command.get(chat_id)
    activities = task_activity.get(chat_id, [])
    ...
    if command:
        lines.extend(["", f"💻 `{_shorten(command, 220)}`"])

    if files:
        lines.extend(["", "📁 Files"])
        for path in files[-5:]:
            lines.append(f"• `{_shorten(path, 100)}`")
...
async def refresh_live_status(bot, chat_id, phase=None, activity=None, force=False):
    """Record activity and refresh the reusable Telegram dashboard."""
    lock = task_activity_lock.setdefault(chat_id, asyncio.Lock())

    async with lock:
        if phase:
            task_phase[chat_id] = phase

        if activity:
            history = task_activity.setdefault(chat_id, [])
            if not history or history[-1] != activity:
...
def describe_event(event):
    """Translate OpenCode events into useful human-readable Telegram activity."""
    ...
    if event_type == "session.status":
        ...
        if status_type == "busy":
            return {"phase": "Thinking", "activity": "🧠 Agent is thinking"}

        if status_type == "idle":
            return {"phase": "Finishing", "activity": "🏁 Agent turn finished"}
    ...
    if event_type == "tool.execute.before":
        ...
        tool_text = _tool_label(tool)
        command = (
            properties.get("input", {}).get("command")
        ...
        result = {
            "phase": "Executing",
            "tool": tool_text,
            "activity": f"⚙️ {tool_text}",
        }

        if command:
            result["command"] = _shorten(command, 220)
    ...
    if event_type == "tool.execute.after":
        ...
        return {
            "phase": "Working",
            "activity": f"✅ Finished `{str(tool).split('/')[-1]}`",
        }
    ...
    if event_type == "file.edited":
        ...
        return {
            "phase": "Editing project",
            "activity": f"🛠️ Edited `{path}`",
            "file": path,
        }

    if event_type in ("file.created", "file.written"):
        path = (
            properties.get("path")
            or properties.get("file")
            or "file"
            ...
            async with httpx.AsyncClient(timeout=None) as client:
                async with client.stream(
                    ...
                    async for raw_line in response.aiter_lines():
                        ...
                        event_info = describe_event(data)
                        if event_info:
                            task_tool[chat_id] = event_info.get(
                                "tool", task_tool.get(chat_id)
                            )
                            task_command[chat_id] = event_info.get(
                                "command", task_command.get(chat_id)
                            )

                            file_path = event_info.get("file")
                            if file_path:
                                task_files.setdefault(chat_id, []).append(file_path)

                            await refresh_live_status(
                                bot,
                                chat_id,
                                phase=event_info.get("phase"),
                                activity=event_info.get("activity"),
                            )
...
async def run_agent(
    ...
    memories = await search_memory(prompt)
    ...
    # Reset live dashboard state for this task.
    task_started_at[chat_id] = time.monotonic()
    task_activity[chat_id] = []
    task_phase[chat_id] = "Starting"
    task_tool.pop(chat_id, None)
    task_files[chat_id] = []
    task_command.pop(chat_id, None)
...
- On Windows, use PowerShell syntax for shell commands.
- Never use bash-only syntax such as `&&` or `||`.
- For long-running processes, do not block waiting forever. Start them in the appropriate background/non-blocking way and verify they are running.
- Keep working until the requested operation is actually complete.
- Give a useful final report containing what you did, what you verified, and any important URLs, commands, or errors.
...
""".strip()
    ...
    try:
        await refresh_live_status(
            ...
            activity="🔎 Preparing the coding task",
        )

        # /message waits until the OpenCode agent turn is actually complete.
        # The global SSE listener provides live activity while this awaits.
        ...
        if not final:
            await refresh_live_status(
                bot,
                chat_id,
                phase="Reading final result",
                activity="📖 Fetching the completed OpenCode response",
            )
        ...
        try:
            ...
            if changed:
                task_files[chat_id].extend(changed)
...
async def status(
    ...
    if running and user_id in task_started_at:
        dashboard = _render_status(update.effective_chat.id)
        await update.message.reply_text(
            dashboard
            + "\n\n"
